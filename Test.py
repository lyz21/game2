...
# 用subsurface剪切读入的图片
hero_surface = []
hero_surface.append(shoot_img.subsurface(pygame.Rect(0, 0, 102, 96)))
hero_surface.append(shoot_img.subsurface(pygame.Rect(0, 96, 102, 32)))
hero_pos = [200, 500]
...
while True:
    ...
    # 改变飞机图片制造动画
    # ticks // (ANIMATE_CYCLE // 2),其中ticks一直小于ANIMATE_CYCLE，取整结果为0-1，因此ANIMATE_CYCLE//2，取整结果则为0-2
    if ticks >= ANIMATE_CYCLE:
        ticks = 0
    hero.image = hero_surface[ticks // (ANIMATE_CYCLE // 2)]  # // 符号为向下取整
